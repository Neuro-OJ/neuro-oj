...
e2eTest("[e2e/messaging] Setup", async () => {
    if (!isE2E) return;
    await waitForServer();
...
e2eTest("[e2e/messaging] 2.1 用户间创建新会话返回 201", async () => {
    if (!isE2E) return;
...
e2eTest("[e2e/messaging] 2.2 已存在会话返回 200", async () => {
    if (!isE2E) return;
    const res = await apiPost(
      "/api/v1/conversations",
      { other_user_id: userBId },
...
e2eTest("[e2e/messaging] 2.3 自聊返回 400", async () => {
    if (!isE2E) return;
    const res = await apiPost(
      "/api/v1/conversations",
...
e2eTest("[e2e/messaging] 2.4 不存在用户返回 404", async () => {
    if (!isE2E) return;
    const res = await apiPost(
      "/api/v1/conversations",
...
e2eTest("[e2e/messaging] 2.5 发送消息返回 201", async () => {
    if (!isE2E) return;
    const res = await apiPost(
...
e2eTest("[e2e/messaging] 2.6 空内容被拒返回 400", async () => {
    if (!isE2E) return;
    const res = await apiPost(
      `/api/v1/conversations/${convId}/messages`,
      { content: "" },
...
e2eTest("[e2e/messaging] 2.7 消息列表返回已发送消息", async () => {
    if (!isE2E) return;
...
e2eTest("[e2e/messaging] 2.8 B 标记已读", async () => {
    if (!isE2E) return;
    const res = await apiPost(
...
e2eTest("[e2e/messaging] 2.9 未读计数为非负整数", async () => {
    if (!isE2E) return;
    const res = await apiGet(
...
e2eTest("[e2e/messaging] 2.10 会话列表包含该会话", async () => {
    if (!isE2E) return;
...
e2eTest("[e2e/messaging] 2.11 A 删除消息返回 204", async () => {
    if (!isE2E) return;
...
e2eTest("[e2e/messaging] 2.12 A 视角消息不可见，B 视角仍可见", async () => {
    if (!isE2E) return;
    // A 视角
...
e2eTest("[e2e/messaging] 2.13 非参与者发送消息返回 404", async () => {
    if (!isE2E) return;
    const ts = Date.now().toString(36);
...
e2eTest("[e2e/messaging] 2.14 私信 SSE 连接可建立", async () => {
    if (!isE2E) return;
...
e2eTest("[e2e/messaging] 2.15 单会话未读计数 ≥0", async () => {
    if (!isE2E) return;
    const res = await apiGet(
...
e2eTest("[e2e/messaging] 2.16 非参与者无法查看未读计数", async () => {
    if (!isE2E) return;
    const ts = Date.now().toString(36);
    const strangerToken = await registerUser(
      "msg_unr_" + ts,

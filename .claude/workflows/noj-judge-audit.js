export const meta = {
  ...
  description:
    "全面审查 noj-judge，15 个 finder + 10 个 adversarial verifier + 聚合",
  phases: [
...
const FINDERS = [
  {
    ...
    prompt:
      `在 ${ROOT} 中搜索所有 unsafe 代码块、裸指针操作、FFI 调用、以及可能导致内存不安全的模式。检查：
...
const allVerdicts = verifiedChunks.flat().filter(Boolean).flatMap((r) =>
  r.verdicts
);
...
log(
  `验证完成: ${realIssues.length} 个真阳性, ${falsePositives.length} 个假阳性`,
);

...
本目录提供 Prometheus
抓取配置（`prometheus.yml`）、告警规则（`noj-alerts.yml`）、 Alertmanager
配置模板（`alertmanager.yml.example`）与 Grafana
仪表盘（`grafana-dashboard.json`）。
...
- `noj-llm-gateway`：`noj-llm-gateway:8001/metrics`（内部网络；P1
  起由网关暴露）。
- `node`（可选）：宿主机 node_exporter，含备份/演练 textfile 指标。

## 3. node_exporter 与备份新鲜度指标

...
**Spec 引用：** `dev-docs/superpowers/specs/2026-07-25-openspec-archive-cleanup-design.md`

## Global Constraints

- 本计划**不动任何运行时模块**：noj-core / noj-ui / noj-judge / 数据库 migrations / GitHub Actions / docker-compose 全部零改动。
- 仅允许修改/创建 `openspec/`、`dev-docs/superpowers/plans/`（本文件）三个区域。
...
Run: `git log --name-only --oneline openspec/ 2>&1 | grep -vE '^(openspec/|dev-docs/superpowers/|commit [a-f0-9]+|Author|Date|\s*$|Merge)' | sort -u`
Expected: 输出仅 `openspec/`、`dev-docs/superpowers/specs/`、`dev-docs/superpowers/plans/` 三个目录的文件。无 noj-core / noj-ui / noj-judge 文件改动。

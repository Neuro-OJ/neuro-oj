...
- [x] 每个 `test-*.sh` 覆盖的行为都有 TS 测试（覆盖清单可核对）
      —— **已补做**（T26）：从基线提交 `3fab63a5` **机器提取**全部 150 条 bash 断言，
      逐条在 TS 侧定位对应测试，产出
      [`2026-09-19-noj-cli-bash-parity-map.md`](../../../dev-docs/unattended/2026-09-19-noj-cli-bash-parity-map.md)。
      结论：除"监控/告警"（属其他模块）与 5 项随 R4/形态变更移除的行为外，全部有对应；
      另有 **3 处有意语义反转**（drill 快照形态、生产目录特征、备份形态），已逐条说明。

...
本文档定义 NOJ 文档与代码中的规范术语。每个词条给出定义、对应的代码字段名（如存在）以及"易混淆"提示。结果状态（`Accepted` / `WrongAnswer` 等）见[结果状态](result-status.md)。
...
NOJ 评分的基本单位：Evaluator 通过 `SolutionRunner` 对用户函数发起的一次 RPC 调用。与传统 OJ 的"测试点"对应，但语义不同——NOJ 的用例数据格式由题目自定，评分以调用结果（返回值或调用错误）为准。
...
`evaluate.py` 读取的评分输入材料。NOJ 不规定格式；内置样例题使用 `visible.jsonl` / `hidden.jsonl` 约定，出题人可用 `cases/*.json`、SQLite、CSV 等任何方式组织。
...
出题人愿意向做题人展示输入、期望与实际结果的用例。内置样例题约定存放于 `visible.jsonl`，但 NOJ 不强制该文件名或数据格式。
...
出题人不希望直接暴露给做题人的评分用例。内置样例题约定存放于 `hidden.jsonl`，但 NOJ 不强制该文件名或数据格式。出题人应避免向用户泄露隐藏输入和标准答案。
...
NOJ 面向大模型能力评测（LMCC）的评测对象。当前 Python 运行时中，用户以函数形式暴露能力，由 Evaluator 调用；`noj_solution_sdk.call_capability` 为预留的占位能力（当前不支持 Solution 到 Evaluator 的能力调用）。

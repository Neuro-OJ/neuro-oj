...
运行 Evaluator 或 Solution 的 Docker 镜像，如 `noj-evaluator-python`、`noj-solution-python`（SDK 构建脚本默认打 `:latest` 标签）。镜像是否可用由[镜像白名单](#judge-image-whitelist)（`judge_images` 表）决定。

...
Deno.test("log-format: 生产环境消息插值位置也脱敏", () => {
  {
    // 显式注入 production：不改写进程级 env（--parallel 安全）
    ...
    const formatted = makeJsonFormatter({ production })(records[0]!);
...
Deno.test("log-format: 生产环境敏感键整值抹除", () => {
  {
    ...
    const production = true;
    assertEquals(
      redactValueByKey("email", "a@b.com", production),
      "[redacted]",
    );
    assertEquals(
      redactValueByKey("api_key", "sk-123", production),
      "[redacted]",
    );
    assertEquals(redactValueByKey("score", 9500, production), "[redacted]");
    assertEquals(redactValueByKey("status", "ok", production), "ok");
  }
});
...
Deno.test("log-format: 开发环境不脱敏", () => {
  {
    ...
    assertEquals(redactValueByKey("submission_id", full, production), full);
  }
});
...
Deno.test("log-format: request_id 不被 *_id 通配规则误截断", () => {
  ...
  // 契约 §1 已由 pretty 侧自行取前 8 字符控制展示长度。
  {
    ...
    assertEquals(
      redactValueByKey("request_id", uuid, production),
      uuid,
      "关联标识必须完整保留，否则生产环境无法串联请求",
    );
    // 同一通配规则下的业务实体 id 仍必须截断——例外只开给关联标识
    assertEquals(
      redactValueByKey("submission_id", uuid, production),
      "550e8400...",
    );
    assertEquals(redactValueByKey("trace_id", uuid, production), "550e8400...");
  }
});
...
Deno.test("log-format: 自引用结构 fail-closed（不死循环，不留明文）", () => {
  {
    ...
    cyclic.self = cyclic;
    const redacted = redactValueByKey("payload", cyclic, production);
    const text = JSON.stringify(redacted);
...
Deno.test("log-format: 超深嵌套 fail-closed（不原样放行明文）", () => {
  {
    ...
    const text = JSON.stringify(redactValueByKey("d", deep, production));
    assertStrictEquals(
      text.includes("hunter2"),
      false,
...
Deno.test("log-format: 兄弟节点共享同一对象不被误判为环", () => {
  {
    ...
    const text = JSON.stringify(
      redactValueByKey("g", { a: shared, b: shared }, production),
    );
    assertStrictEquals(text.includes("hunter2"), false);
    assertStrictEquals(
...
Deno.test("log-format: 生产环境下 Error 标记在脱敏后仍保留", () => {
  {
    ...
    const after = redactValueByKey("error", new Error("boom"), production);
    assertStrictEquals(

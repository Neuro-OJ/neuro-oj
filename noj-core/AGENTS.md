...
- `createConsumerRedis()` — 独立连接，用于 BRPOP 阻塞等待结果（`lazyConnect: true`，指数退避永不停止），也用于 Judge RPC handler
...
- 启动时创建独立 Redis 连接（`createConsumerRedis()`），BRPOP 监听 `noj:rpc:v1:judge:core`
...
7. **启动 Judge RPC 处理器** — 使用独立连接监听 `noj:rpc:v1:judge:core`，响应 `get_image_allowlist` 等请求
8. **启动 HTTP 服务**

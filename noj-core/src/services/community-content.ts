 ...
 *
 * 跨子域依赖：本文件从 ./community-helpers.ts 导入 featureForType，
 * 并内含 createNotification（所有调用方均在本子域内）。
 */
...
import { createActivity } from "./community-social-feed.ts";
import { createNotification } from "./community-moderation.ts";
import { featureForType } from "./community-helpers.ts";
import type {
  CommunityConfig,
  CommunityPostInput,
  CommunityPostStatus,

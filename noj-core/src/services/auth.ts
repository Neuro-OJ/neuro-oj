 ...
 * 密码强度校验最小长度。
 *
 * 历史上遵循 OWASP 2025+ 建议采用 12 字符以增加破解空间；
 * 当前放宽至 8 字符（仍保留大小写字母 + 数字 + 不得与用户名/邮箱前缀相同
 * 的复杂度要求），在易用性与安全性之间取得平衡。
 */
 ...
 * 6. 不能与邮箱前缀相同
 *
 * issue 64 评论 §6.5 修复：早期仅 length >= 8，弱密码
 * （"12345678"、"password"）可通过；当前仍保留长度下限 + 复杂度组合校验。
 *

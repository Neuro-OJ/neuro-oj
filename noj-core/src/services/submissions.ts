...
export function saveEvaluationResult(result: EvaluationResult): void {
  evaluationResultsStore.set(result.submission_id, result);
}

/**
 * 获取评测结果
 */
export function getEvaluationResult(submissionId: string): EvaluationResult | undefined {
  return evaluationResultsStore.get(submissionId);
}

 ...
 * 在"存量脏数据"前提下运行一段逻辑。
 *
 * 迁移 0084 用 `NOT VALID` 添加形态约束：**存量**非规范行会被保留（这正是
 ...
 * 一条这样的行即可让门控查询 500（可用性事故），迁移 0084 的 UPDATE 也会因此回滚。
 *
...
/**
 * 评审 #3 回归（迁移）：迁移 0084 的规范化 UPDATE 不得对未验证值直接 cast。
 *
 ...
 * `2026-13-01T00:00:00` 通过预筛却无法转换 → 整个迁移事务回滚 → core 起不来。
 *
 * 本用例**直接读取 `drizzle/0084_*.sql` 文件内容并执行**（而非在测试里重写一份
 * 等价 SQL）：这样才能在有人改回旧迁移时真正失败，而不是"测试自说自话"。
 */
Deno.test({
  name: "contest-window(评审#3): 真实迁移 0084 对语义非法行不抛错且原样保留",
  sanitizeResources: false,
  sanitizeOps: false,
  fn: async () => {
    await reset();
    ...
    for await (const entry of Deno.readDir(migrationsDir)) {
      if (entry.isFile && entry.name.startsWith("0084_")) {
        migrationSql = await Deno.readTextFile(
          new URL(entry.name, migrationsDir),
        );
      }
    }
    assertEquals(migrationSql.length > 0, true, "未找到 0084 迁移文件");
...
/**
 * 第三道防线（迁移 0084 / schema-ddl）的回归测试。
 *
...
Deno.test({
  ...
  fn: async () => {
    ...
    // 迁移 0084 的 NOT VALID 约束对**新写入**生效：非规范形态必须被拒绝
    let rejected = false;
    try {

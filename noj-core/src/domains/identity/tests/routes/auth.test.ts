...
import { _resetLoginBackoffForTest } from "../../services/security/loginThrottle.ts";
import {
  type LogRecord,
  resetLogSink,
  setLogSink,
...
if (hasRedis) {
  try {
    const redisModule = await import("../../../../shared/mq/connection.ts");
...
async function ensureRedisConnected() {
  resetRedisForTest();
  const { connectRedis } = await import("../../../../shared/mq/connection.ts");
  try {
    await connectRedis();
  } catch (e) {

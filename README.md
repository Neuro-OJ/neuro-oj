...
[![CI](https://github.com/anomalyco/neuro-oj/actions/workflows/ci.yml/badge.svg)](https://github.com/anomalyco/neuro-oj/actions/workflows/ci.yml)
...
- **Pool 模式（默认）** — 预创建容器池，RAII 自动回收，支持健康检查、自动扩缩容、Prometheus 指标暴露
- **Semaphore 模式（回退）** — 每任务创建/销毁容器，通过信号量限制并发
...
6 张表：`users`, `problems`, `categories`, `problems_categories`, `submissions`, `evaluation_results`，支持 U/P 双题库（用户题/主题题）和分类体系。
...
├── openspec/          # OpenSpec 规范驱动开发（26 specs + 变化管理）
├── scripts/           # 构建与维护脚本
...
- **e2e.yml** — 全链路管道测试（52 API 测试 + 12 Docker 沙箱测试，5-15min）

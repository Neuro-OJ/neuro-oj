...
资源要求分为“仅能启动/诊断”和“可承载公测”两种语义。单次低负载观测不代表整机最低要求；
开启 Judge、批量提交或 LLM 后，必须按固定版本和并发重新做容量验收。

| 资源 | 仅能启动/诊断（硬门槛） | 低并发公测起始建议 | 推荐要求（正式生产） |
| --- | --- | --- | --- |
| CPU | ≥ 2 vCPU | ≥ 4 vCPU（待容量报告确认） | ≥ 4 vCPU，并为评测峰值留余量 |
| 内存 | ≥ 2 GiB（`noj-cli doctor` 硬门槛） | ≥ 8 GiB（Judge 同机时） | ≥ 8 GiB，按峰值和增长空间扩容 |
| Swap | 建议 ≥ 2 GiB | ≥ 4 GiB | ≥ 4 GiB |
| 磁盘 | 至少保持 10 GiB 可用 | 至少保持 40 GiB 可用 | 按镜像、日志、备份和对象增长规划 |
| Docker 存储 | 至少预留 5 GiB | 至少预留 15 GiB | 按评测镜像和缓存增长规划 |

曾记录的 264 MiB 是 NOJ 低负载进程观测值，不能作为包含 PostgreSQL、Redis、对象存储、Nginx、
core、UI 和 Judge 沙箱的完整生产主机最低要求。NOJ 当前持久化数据卷约 66 MiB，生产镜像（含
Evaluator、Solution 评测镜像）逻辑大小约 2.75 GiB；实际磁盘还会受到旧版本镜像、评测缓存、日志、
备份和同机其他服务影响。容量测试方法见[公测容量基线](./noj-docs/docs/operators/capacity-baseline.md)。
...
NOJ 使用统一的「暖纸评测风」品牌视觉：暖纸底、墨字、蓝黑墨品牌蓝 `#1B2B4A` 与评测信号绿 `#00d68a`。设计 token 与使用规范见 [`dev-docs/design/noj-design-tokens.md`](./dev-docs/design/noj-design-tokens.md)。

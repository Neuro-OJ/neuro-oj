...
本变更吸收「纸墨 + 信号色」这类工业编辑方法论（参考《明日方舟：终末地》与 `dsh-theme-endfield` 的部分设计思路），但**不直接复制**参考元素；所有颜色、纹理、名字和装饰都替换为 NOJ 自己的语义：暖纸、墨字、蓝黑墨品牌蓝、评测信号绿。
...
- `noj-brand-design-system`: 定义 NOJ 品牌设计 token、UI 风格约束和文档同步要求。
...
- 修改 `noj-ui` 布局/全局样式，加入圆角、字号、数字等宽等样式约束。

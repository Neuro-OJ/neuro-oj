...
为评测结果添加时间 (`time_ms`) 和内存峰值 (`memory_kb`) 字段，当前二者始终为 `null`。
...
- **内存**：exec 结束后在容器内执行 `cat /sys/fs/cgroup/memory/memory.max_usage_in_bytes`（cgroup v1）或 `memory.peak`（cgroup v2）

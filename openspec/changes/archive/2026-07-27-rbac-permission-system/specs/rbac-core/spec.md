...
**权限判断 SHALL 依赖 `is_admin` 布尔属性，不依赖角色名称。** 角色名称可由管理员重命名（如 "admin" 改为 "超级管理员"），`is_admin` 标记才是权限判断的权威依据。
...
注册时通过查询 `SELECT * FROM roles WHERE is_default = true` 获取默认角色，**不依赖角色名称**。角色名称可由管理员重命名（如 "user" 改为 "普通用户"），`is_default` 标记才是自动分配的依据。
...
若用户拥有 `is_admin=true` 的角色，函数 SHALL 返回一个特殊的标记而不是展开所有权限（实际权限检查通过 `is_admin` 标记短路）。
...
系统 SHALL 提供 `checkPermission(userId, permission)` 函数，检查用户是否拥有指定权限。若用户拥有 `is_admin=true` 的角色则直接返回 true，否则调用 `getUserPermissions` 后进行 `Set.has()` 检查。

#### Scenario: 用户拥有请求的权限
- **WHEN** `checkPermission(userId, "problem:create")` 且用户拥有该权限
- **THEN** 返回 true

#### Scenario: 用户不拥有请求的权限
- **WHEN** `checkPermission(userId, "problem:create_p")` 且普通用户不拥有该权限
- **THEN** 返回 false

#### Scenario: admin 用户直接放行
- **WHEN** `checkPermission(userId, anyPermission)` 且用户拥有 `is_admin=true` 的角色

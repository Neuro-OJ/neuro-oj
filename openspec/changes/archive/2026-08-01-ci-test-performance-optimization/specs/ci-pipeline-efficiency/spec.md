...
- **THEN** admin 改密竞态由 `helper.ts` `loginAndChangePassword` 并发兜底
  （change-password 失败时改用 newPassword 登录重试）

## MODIFIED Requirements

### Requirement: CI Job 独立拆分（ci-optimization）

原"`core-fmt` / `core-lint` / `judge-fmt` / `judge-clippy` / `judge-test`
各自独立 Job"的要求 REMOVED：`core-fmt` / `core-lint` 并入
`core-quick-check`（fmt + lint + typecheck 一体）；`judge-fmt` /
`judge-clippy` / `judge-test` 并入 `judge-check`（见 ADDED Requirement
"Rust job 合并为 judge-check 单 job"）。`judge-e2e` 从仅
`workflow_dispatch` 改为默认 push + PR + manual 全跑。

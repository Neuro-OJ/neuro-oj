...
**资源测量集成**：
`execute_in_container` 中用 `Instant::now()` 记录 exec 前后差值作为 `time_ms`。
exec 结束后另启一个 `docker exec cat memory.max_usage_in_bytes` 读取 cgroup 内存峰值作为 `memory_kb`。
两个字段填入 `JudgeResult` 返回给 noj-core。详见 `openspec/changes/judge-resource-metrics/`。

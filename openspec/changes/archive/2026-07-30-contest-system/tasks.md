...
- [x] 11.2 修复竞赛界面深色背景 Token，确保题目标签和深色区域的文字对比度
- [x] 11.3 创建、编辑或删除竞赛成功后自动重新加载竞赛管理页面
- [x] 11.4 恢复 `useToast` 的 `showToast` 兼容入口，修复竞赛及其他管理页面通知调用
- [x] 11.5 补充 OI 竞赛进行中排名的登录、参赛者与管理员可见性测试
- [x] 11.6 扩展竞赛生命周期 E2E：覆盖密码错误拒绝与无 Judge 时的报名成功路径

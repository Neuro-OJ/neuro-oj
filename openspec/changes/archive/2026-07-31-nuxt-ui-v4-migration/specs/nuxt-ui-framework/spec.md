...
定义 Neuro OJ 前端（noj-ui）以 Nuxt UI v4 作为通用组件框架的规范——组件使用
约束、主题 token 映射、弹窗/Toast 行为与组件自定义边界。
...
noj-ui SHALL 将现有 `--c-*` 设计 token 映射到 Nuxt UI 主题变量（`--ui-primary`、
`--ui-bg`、`--ui-border`、`--ui-text` 等），保持迁移前后视觉一致。

#### Scenario: 主色一致

- **WHEN** 渲染 Nuxt UI 主色组件
- **THEN** 主色解析为 `var(--c-primary)`

#### Scenario: 暗色编辑器保留

- **WHEN** 进入编辑器暗色上下文（`.editor-dark`）
- **THEN** `--ui-*` 变量随 `--c-*` 覆盖联动，组件自动适配暗色

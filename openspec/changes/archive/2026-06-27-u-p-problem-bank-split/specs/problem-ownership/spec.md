...
系统 SHALL 在 API 响应中提供 `display_id` 字段，格式为 `{type}{number}`。

#### Scenario: U 型题目标识
- **WHEN** API 返回 type='U', number=42 的题目
- **THEN** display_id 为 "U42"

#### Scenario: P 型题目标识
- **WHEN** API 返回 type='P', number=1001 的题目
- **THEN** display_id 为 "P1001"
...
#### Scenario: display_id 不存在
- **WHEN** 用户请求 `GET /api/v1/problems/P99999`
- **THEN** 系统返回 HTTP 404

### Requirement: 基于 type + owner 的权限控制

系统 SHALL 在服务层（services/problems.ts）实现基于题目类型和所有者的权限判断。

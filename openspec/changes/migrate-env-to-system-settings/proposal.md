...
- **扩展 DB-backed 设置注册表**：将约 22 个应用层配置项从 env-only 迁移为 DB-backed，纳入 `SETTING_DEFINITIONS` 注册表，使管理员可通过 `PUT /api/v1/admin/settings/:key` 即时修改
- **重构配置读取路径**：所有被迁移的配置项，代码中不再直接调用 `Deno.env.get()`，改为通过 `getSetting()` 读取（走 DB → env → default 兜底链）
- **env-only 白名单精简**：仅保留真正的基础设施配置项（数据库连接、Redis 连接、JWT 密钥、端口、运行环境、管理员引导凭据、bcrypt 轮数等），这些配置项在 DB 可用之前就需要，无法 DB-backed
- **新增 integer 类型**：为速率限制等数值型配置新增 `integer` setting type，支持 `min`/`max` 范围校验
- **新增 storage 类别**：S3 相关配置（endpoint、region、凭证、bucket 等）作为 DB-backed 设置项纳入 `storage` 类别
- **管理后台 UI 扩展**：前端设置页面新增 integer 类型输入控件（number input），新增 storage 分组，扩展可编辑区域至约 22 项
...
- **noj-core `lib/storage/factory.ts`**：S3 配置及 `STORAGE_PROVIDER` 改为通过 `getSetting()` 读取（在 `initSystemSettings()` 之后调用）

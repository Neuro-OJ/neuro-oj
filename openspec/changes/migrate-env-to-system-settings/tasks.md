...
- [ ] 1.2 `lib/settings-registry.ts`：`SETTING_DEFINITIONS` 扩展至约 20 项（按 design.md Decision 1 表格），覆盖 auth、maintenance、email、rate_limit、other 类别
- [ ] 1.3 `lib/settings-registry.ts`：`validateRegistry()` 新增 integer 类型的 `min ≤ max` 校验
...
- [ ] 3.8 全局搜索确保所有被迁移的 env key 不再通过 `Deno.env.get()` 直接读取（保留 infrastructure 项）

## 4. env-only 白名单精简

- [ ] 4.1 `lib/env-snapshot.ts`：从 `ENV_ONLY_DEFINITIONS` 中移除已迁移至 DB-backed 的 15 个条目，仅保留 infrastructure 项（DATABASE_*、REDIS_URL、JWT_SECRET、PORT、NOJ_ENV、STORAGE_PROVIDER、S3_*、ADMIN_EMAIL、ADMIN_PASS、BCRYPT_SALT_ROUNDS）

## 5. 前端适配

- [ ] 5.1 `noj-ui/pages/admin/settings.vue`：新增 integer 类型输入控件（`<input type="number" step="1" :min="def.min" :max="def.max">`）
- [ ] 5.2 `noj-ui/pages/admin/settings.vue`：可编辑设置表格按 category 分组展示（auth / maintenance / email / rate_limit / other），每组带标题

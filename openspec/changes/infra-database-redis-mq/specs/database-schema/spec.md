...
| support_package_path | TEXT    |                            | 支持包 zip 路径      |
...
- **THEN** 题目记录包含完整的评测环境配置，支持包路径可为空（待后续上传）
...
- **WHEN** noj-core 启动且 `data/noj.db` 不存在
- **THEN** 系统自动创建数据库文件并执行所有待执行的 migration，创建 4 张表

#### Scenario: 已有数据库启动

- **WHEN** noj-core 启动且数据库文件已存在，所有 migration 已执行
- **THEN** 系统跳过迁移，正常启动

...
- 交互式配置使用权限为 `600` 的临时文件承接所有输入。用户确认“写入”后才通过原子替换更新 `.env.prod`；取消或异常退出时删除临时文件，保留原配置。没有可交互终端时继续保留模板文件并返回非零状态，方便自动化场景显式处理。
- 邮件服务提示根据已有配置区分含义：没有旧配置时回车选择“暂不配置”；已有阿里云或腾讯云配置时回车表示继续使用，输入 `skip` 才会停用，避免用户因误解而进入密钥输入。

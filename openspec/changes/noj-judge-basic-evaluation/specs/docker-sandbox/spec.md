...
系统 SHALL 从两个来源之一获取支持包（`support_package_base64` 优先，`support_package_path` 兜底），将 zip 解压到临时目录。若两者均为空，跳过此步骤。
...
- **WHEN** support_package_base64 为空但 support_package_path 有效且文件存在
- **THEN** 读取该 zip 文件并解压到 `{work_dir}/` 下

#### Scenario: 支持包来源为空

- **WHEN** support_package_base64 和 support_package_path 均为空

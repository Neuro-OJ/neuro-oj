...
| | 2FA | 支持 | 无 | 锦上添花 |
| | 用户主页 | 含统计/图表 | 含题数/通过率/排名 | |
| | 个人简介(bio) | 支持 | 支持 | |
| | 站内信/通知 | 用户间+系统通知 | 站内私信（conversations+messages） |  |
| **题目管理** | CRUD | 完整 | 完整 | |
...
| | SSE/WebSocket | 完整 pub/sub | SSE 推送 + 轮询回退 | 部分实现 |
...
Phase 1 就建立了完整测试金字塔：单元测试（37 测试文件）、跨模块 E2E（70 测试）、Docker 沙箱 E2E（6 套件）。Hydro OJ 作为成熟项目测试覆盖反而较弱。

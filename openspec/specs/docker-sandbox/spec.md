...
- **THEN** ensure_image_local 返回错误，提示用户先构建镜像；noj-judge 使用本地构建策略（如 `noj-judge-python` 由 Dockerfile 提前构建），不从远程自动拉取
...
- **WHEN** 容器运行时间超过 time_limit_ms + 5s
- **THEN** 系统调用 Docker API kill 容器，capture_container_logs 返回 exit_code=-1

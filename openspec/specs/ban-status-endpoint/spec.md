...
定义封禁状态查询端点，为前端提供统一的 IP 封禁和用户封禁状态检查能力，支撑全局封禁 Banner 展示。
...
- `user_banned` SHALL 为 `true` 当且仅当存在有效 JWT 且对应用户存在活跃封禁
...
- **WHEN** 已登录用户存在活跃封禁（`unbanned_at IS NULL`）
- **AND** 客户端 IP 未被封
...
- **WHEN** `user_bans` 中活跃记录 `banned_until < now`

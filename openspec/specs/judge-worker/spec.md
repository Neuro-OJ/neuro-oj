...
每个 noj-judge 实例 SHALL 限制同时执行的评测任务数量。该上限 SHALL 通过
`JUDGE_MAX_CONCURRENT_JUDGES` 环境变量配置；变量缺失、为零或不是正整数时 MUST 使用
安全的有限默认值。任务拉取不得无限制地将 Redis backlog 转换为已启动的 Docker 评测任务。

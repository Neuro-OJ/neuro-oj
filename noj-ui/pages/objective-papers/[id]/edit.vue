...
useHead({ title: '编辑套卷 - Neuro OJ' })

const route = useRoute()
const paperId = route.params.id as string
const { getPaper, updatePaper, deletePaper, listQuestions, createQuestion, updateQuestion, deleteQuestion } =
  useObjective()
...
async function onSaveMeta() {
  ...
  try {
    await updatePaper(paperId, { title: title.value.trim(), description: description.value.trim() })
...
async function onSaveQuestion() {
  ...
  try {
    if (e.id === null) {
      ...
      await updateQuestion(e.id, payload)
      toast.success('小题已更新')
    }
    closeEditor()
      ...
      <div v-if="paper" class="mx-auto flex max-w-[860px] flex-col gap-4">
        <header class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-text-secondary">{{ paper.display_id }}</p>
          </div>
          <UButton color="neutral" variant="outline" icon="i-lucide-arrow-left" :to="`/objective-papers/${paper.id}`">
            返回答题页
          </UButton>
        </header>
        ...
        <section v-if="editing" class="rounded-xl border border-primary/30 bg-white p-5">
          ...
          <div class="flex flex-col gap-3">
            ...
            <!-- 选项（判断题固定对/错） -->
            <UFormGroup v-if="editing.type !== 'judge'" label="选项与答案">

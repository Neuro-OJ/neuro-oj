...
noj-core 当前测试隔离依赖 `resetDbForTest()` 的 `TRUNCATE + re-seed`，全树约 259 次调用，是串行测试的主要开销之一。此前已通过 PGlite 模板、bcrypt cost 降低、reset 轻量化等优化，但每次用例仍需要清空全表并重新播种。

本设计引入**事务回滚级隔离**：每个测试用例在一个数据库事务中执行，用例结束统一 `ROLLBACK`，从而让用例内 `resetDbForTest()` 变成 no-op，大幅降低测试开销。
...
- 不改变生产代码行为。
- 不引入并行分片。
- 不要求修改所有测试文件；只提供全局 preload 包装 + 文件级 opt-out。
